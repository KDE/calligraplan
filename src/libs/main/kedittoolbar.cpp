...
public:
    ...
    {
#if 0
        qDebug() << "XmlData" << this << "type" << s_XmlTypeToString[m_type] << "xmlFile:" << m_xmlFile;
        for (const auto element : m_barList) {
...
public:
     ...
     * to the KoEditToolBar constructor.
...
{
public:
    KEditToolBarPrivate(KoEditToolBar *qq)
        : q(qq)
    {
    }
...
KoEditToolBar::KoEditToolBar(KActionCollection *collection, QWidget *parent)
    : QDialog(parent)
...
KoEditToolBar::KoEditToolBar(KXMLGUIFactory *factory, QWidget *parent)
    : QDialog(parent)
    , d(new KEditToolBarPrivate(this))
{
...
void KoEditToolBar::setResourceFile(const QString &file, bool global)
{
    d->m_file = file;
    d->m_global = global;
...
KoEditToolBar::~KoEditToolBar()
{
    s_defaultToolBarName()->clear();
}

void KoEditToolBar::setDefaultToolBar(const QString &toolBarName)
{
...
void KoEditToolBar::setGlobalDefaultToolBar(const char *toolbarName)
{
...
void KEditToolBarWidget::save()
{
    ...
    for (const auto &xmlFile : std::as_const(d->m_xmlFiles)) {
        ...
        //qDebug() << xmlFile.domDocument().toString();

        //qDebug() << "Saving " << xmlFile.xmlFile();
...
void KEditToolBarWidget::rebuildKXMLGUIClients()
{
    ...
    //qDebug()<<Q_FUNC_INFO << "rebuilding the gui"<<clients;
    for (KXMLGUIClient *client : clients) {
...
void KoEditToolBar::showEvent(QShowEvent *event)
{
...
void KoEditToolBar::hideEvent(QHideEvent *event)
{
